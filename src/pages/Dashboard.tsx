...
import { Plus, Calendar, Clock, TrendingUp, Upload, DollarSign, AlertCircle, CalendarDays } from "lucide-react";
...
export default function Dashboard() {
  ...
  const financeStats = useMemo(() => {
    ...
    const faturamentoMes = thisMonthFinanceiro.reduce((sum, l) => sum + (Number(l.valor_honorarios) || 0), 0);
    const totalPendente = financeiro.filter(l => l.status === "pendente").reduce((sum, l) => sum + (Number(l.valor_honorarios) || 0), 0);
    return {
      faturamentoMes,
      totalPendente
    };
  ...
  const percentChange = lastMonthLaudos > 0 ? Math.round((thisMonthLaudos - lastMonthLaudos) / lastMonthLaudos * 100) : thisMonthLaudos > 0 ? 100 : 0;

  // Get recent laudos for history table (last 5)
  const recentLaudos = useMemo(() => {
    return [...laudos].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).slice(0, 5);
  }, [laudos]);
  ...
  const upcomingAppointments = useMemo(() => {
    ...
    }).sort((a, b) => new Date(a.dataPericia!).getTime() - new Date(b.dataPericia!).getTime()).slice(0, 5);
    return future;
  }, [laudos]);
  ...
  return <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="shadow-sm">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-sm font-medium text-muted-foreground">Laudos Pendentes (Rascunhos) </p>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-bold text-foreground">
                  R$ {financeStats.faturamentoMes.toLocaleString('pt-BR', {
                  minimumFractionDigits: 2
                })}
                </p>
                <p className="text-xs text-muted-foreground">
                  {financeStats.totalPendente > 0 ? `R$ ${financeStats.totalPendente.toLocaleString('pt-BR')} a receber` : "Clique para gerenciar"}
                </p>
              </div>
              <div className="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <PericiasCalendar pericias={laudos.map(l => ({
        id: l.id,
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <ProximosCompromissosCards appointments={upcomingAppointments} onViewAll={() => navigate('/historico')} />
        <HistoricoRecenteTable laudos={recentLaudos} onDelete={deleteLaudo} onViewAll={() => navigate('/historico')} />
      </div>
